...
if 'data_loaded' not in st.session_state:
    with st.spinner("Loading model and data..."):
        model = load_mnist_model()  # Fixed function call
...
if 'data_loaded' in st.session_state and st.session_state.data_loaded:
    ...
    attack_type = st.sidebar.selectbox("Select Attack", options, 
                                       help="""CopyCatCNN: 🔍 Creates a substitute model by querying the target model with synthetic data and training a neural network to replicate its predictions and decision boundaries,
        ...
        if attack_type == "CopyCatCNN":
            ...
            with st.spinner("⏳ Running " + attack_type + " attack... Please wait"):
                ...
                target_model.compile(optimizer="adam", 
                                   ...
                                   metrics=["accuracy"])
                target_model.fit(train_images_flat, train_labels, epochs=5, verbose=0)
                loss, acc_org = target_model.evaluate(test_images_flat[:5000], test_labels[:5000], verbose=0)
                ...
                stolen_classifier = attack.extract(
                    test_images_flat[5000:], test_labels[5000:],
                ...
                loss, acc_stolen = stolen_classifier.model.evaluate(test_images_flat[:5000], test_labels[:5000], verbose=0)
                acc_drop = acc_org - acc_stolen
                st.success("✅ Functionally Equivalent Extraction completed!")
                
                org_pred = classifier_fee.predict(test_images_flat[:5000])
                stol_pred = stolen_classifier.predict(test_images_flat[:5000])
                if len(org_pred.shape) > 1:  # If probability outputs
                    original_classes = np.argmax(org_pred, axis=1)
                else:
                    original_classes = org_pred
            ...
            with st.spinner("⏳ Running " + attack_type + " attack... Please wait"):
                ...
                y_test_cat = to_categorical(test_labels[nb_stolen:], nb_classes=10)
                loss, acc = classifier_stolen._model.evaluate(test_images[nb_stolen:], y_test_cat, verbose=0)
                loss, acc_org = classifier.model.evaluate(test_images[nb_stolen:], y_test_cat, verbose=0)
