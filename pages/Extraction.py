...
def load_data():
    try:
        with global_lock:
            ...
            # Reduce data size for better performance with multiple users
            max_train_size, max_test_size = 5000, 2500  # Reduced from 10000, 5000
...
def get_model_simple(NUM_CLASSES, session_id=None):
...
def get_model_minimal(NUM_CLASSES, session_id=None):
    """Minimal model for resource-constrained scenarios"""
    try:
        with model_creation_lock:
            model = Sequential([
                ...
                Dense(NUM_CLASSES, activation='softmax')
            ], name=f"minimal_model_{session_id}_{int(time.time())}")
        ...
        st.error(f"Error creating minimal model: {e}")
        return None
    ...
    if 'data_loaded' not in st.session_state:
        ...
        with st.spinner("Loading model and data..."):
            try:
                ...
                st.success(f"✅ Session {st.session_state.session_id} initialized!")
...
resources = get_system_resources()
mem_color = "🔴" if resources['memory_percent'] > 85 else "🟡" if resources['memory_percent'] > 70 else "🟢"
cpu_color = "🔴" if resources['cpu'] > 90 else "🟡" if resources['cpu'] > 70 else "🟢"

st.sidebar.markdown(f"""
**Memory**: {mem_color} {resources['memory_percent']:.1f}%  
**CPU**: {cpu_color} {resources['cpu']:.1f}%  
**Active Users**: ~{MAX_CONCURRENT_ATTACKS - attack_semaphore._value}  
...
options = ["CopyCatCNN", "Knockoff Nets"]  # Removed FEE for now due to complexity
...
if attack_type == "CopyCatCNN":
    ...
    param["batch_size_fit"] = st.sidebar.slider("Batch Size (Training)", 8, 32, 16, 8)  # Reduced
    param["batch_size_query"] = st.sidebar.slider("Batch Size (Query)", 8, 32, 16, 8)  # Reduced
    param["nb_epochs"] = st.sidebar.slider("Training Epochs", 1, 5, 2)  # Reduced
    param["nb_stolen"] = st.sidebar.slider("Number of Samples to Steal", 500, 2000, 1000, 250)  # Reduced
    param["use_probability"] = st.sidebar.checkbox("Use Probability Output", value=True)
    param["model_type"] = st.sidebar.selectbox("Model Complexity", ["Minimal (Faster)", "Simple (Better)"])

elif attack_type == "Knockoff Nets":
    steal_dataset = st.sidebar.selectbox("Dataset for Stealing", ["MNIST Test Set", "CIFAR-10", "Fashion-MNIST"])
    param["batch_size_fit"] = st.sidebar.slider("Batch Size (Training)", 4, 16, 8, 4)  # Reduced
    param["batch_size_query"] = st.sidebar.slider("Batch Size (Query)", 4, 16, 8, 4)  # Reduced
    param["nb_epochs"] = st.sidebar.slider("Training Epochs", 1, 3, 2)  # Reduced
    param["nb_stolen"] = st.sidebar.slider("Number of Samples to Steal", 250, 1000, 500, 125)  # Reduced
    param["use_probability"] = st.sidebar.checkbox("Use Probability Output", value=True)
...
if run_button:
    ...
    if 'attack_running' in st.session_state and st.session_state.attack_running:
        ...
        try:
            ...
            # Choose model based on user preference and system load
            model_func = get_model_minimal if param.get("model_type") == "Minimal (Faster)" else get_model_simple
            ...
            if attack_type == "CopyCatCNN":
                ...
                with st.spinner("⏳ Running Knockoff Nets attack..."):
                    with resource_manager():
                        ...
                        # Simple batch processing without complex progress tracking
