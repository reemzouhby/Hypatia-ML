...
def clamp_nb_stolen(nb_stolen, total_len):
    if nb_stolen >= total_len:
        st.warning(f"Requested nb_stolen ({nb_stolen}) >= available test samples ({total_len}). Clamping to leave at least 1 sample.")
...
if attack_type == "CopyCatCNN":
    ...
    param["reward"] = st.sidebar.selectbox("Reward Strategy", ["cert","div","loss"])
...
if run_button:
    ...
    if attack_type=="CopyCatCNN":
        ...
        if len(org_pred.shape) > 1: original_classes = np.argmax(org_pred, axis=1)
        else: original_classes = org_pred
        if len(stol_pred.shape) > 1: stolen_classes = np.argmax(stol_pred, axis=1)
        else: stolen_classes = stol_pred
        ...
        attack = FunctionallyEquivalentExtraction(classifier_target, nb_epochs=param.get("nb_epochs",5))
        ...
        y_test_cat = to_categorical(test_labels[:nb_stolen], 10)
        loss_org, acc_org = safe_evaluate(classifier_target.model, test_images_flat[:nb_stolen], test_labels[:nb_stolen])
